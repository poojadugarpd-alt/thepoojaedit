...
The homepage's own words (hero headline/lead, rail headings, edit-block copy, newsletter copy) and which products populate its two rails are owner-editable from `/admin/home` and `/admin/collections` respectively — not hard-coded — via `StoreSettings` (`home.content`) and two reserved, catalog-scoped `Collection` rows (`isInternal: true`), so wording and merchandising changes never require a code edit or deploy (D-108/D-109). The same `home.content` setting also carries an owner-editable section order/visibility list and, per media spot, either the automatic product photo or a deliberately uploaded image/short video (a public `site-media` Storage bucket) — the opening headline block is the one section that always renders first (D-110).
