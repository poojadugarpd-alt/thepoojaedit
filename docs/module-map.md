...
| `shipping` | Serviceability, server-priced quotes, shipment creation/reconcile, AWB/label/tracking normalization, NDR/RTO, COD remittance; Shadowfax adapter (single carrier, master v1.1) | Phase 8 -- slice built: `port`/`status`/`shadowfax`/`testing`/`service`.ts, `/api/webhooks/shadowfax`, `/admin/shipments/[id]/label`. Live Shadowfax evidence deferred (blocker #3). |
...
| `invoices` | Financial-year sequence (atomic), immutable legal/line/tax snapshots, async private PDF generation, credit notes | Phase 9 -- slice built (`numbering`/`snapshots`/`pdf`/`service`.ts + `pdf-lib`; `src/lib/documents.ts`; `/order/[orderNumber]/invoice`; `generate-invoice` cron). Owner tax/field review pending. |
| `refunds` | Aggregate refund limits (counting pending), provider refund reconciliation, stable operation keys | Phase 9 -- slice built (`service.ts` over the Phase 7 `createOrderRefund` primitive; `refund.completed` event + credit note; `reconcile-refunds` cron). |
| `returns` | Return/ReturnItem lifecycle, inspection outcomes, explicitly authorized restock (exactly once) | Phase 9 -- slice built (`service.ts`; `restockUnits` key `return-restock:<returnItemId>`; `resolveReturn(REFUND)` -> refund workflow). |
| `notifications` | Versioned template registry, variable validation, consent/eligibility, Resend + WhatsApp adapters, `NotificationDelivery` dedup keys, in-app `AdminNotification` | Phase 10 -- slice built (`templates`/`transports`/`service`/`testing`.ts; `/api/webhooks/{whatsapp,resend}`; `send-notifications` cron; seed rows). Live Resend/Meta evidence deferred (blocker #4). |
| `admin` | `requireAdmin()` / ownership guards, active-role checks from trusted storage, owner bootstrap + revocation, `AdminActivityLog`, `CustomerNote`, bounded bulk actions | Phase 3 (guards) / Phase 11 -- built: `orders`/`tasks`/`customers`/`settings`/`activity`/`bulk`.ts + 11 `/admin/*` pages + `src/features/admin/*`. `resolveTaskChecked` verifies the condition; `runBulk` bounded + per-record results; no raw status setter. |
| `analytics` | Sales/orders by catalog, mixed-order line allocation, COD vs prepaid, captured revenue vs placed vs remittance, low stock | Phase 11 -- built (`getFinancialSummary`/`getOverview`/`getLowStock`; four distinct amounts; line-allocated catalogue revenue; never reads acquisition cost). |
