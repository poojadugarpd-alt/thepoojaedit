...
| --- | --- | --- | --- |
| 1 | **Supabase dev + prod projects** | AC-12 (anon Data API denial, real signed-URL upload, private-bucket download), AC-03/AC-11 live auth, Supavisor+Prisma concurrency proof | create both projects; apply `supabase/policies/*.sql`; wire `NEXT_PUBLIC_SUPABASE_*` + `SUPABASE_SECRET_KEY`; verify Free-tier allowances / pause / backup entitlement against the live account |
