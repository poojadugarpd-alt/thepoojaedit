...
| 6 | **Inngest project — ✅ fixed 2026-09-11 (D-97), pending a live scheduled run to fully close** | the recurring schedules actually firing (they are wired + unit-proven, no live scheduled run yet) | Inngest project + Vercel integration are set up (app `poojaedit`, 10 functions in `src/inngest/functions.ts`, `serve()` route live). The first sync attempt failed against a Vercel *deployment-specific* URL blocked by Vercel Deployment Protection — **not**, as D-94 first concluded, a problem needing the domain cutover: the stable alias `https://thepoojaedit.vercel.app` is publicly reachable right now. Manually synced Inngest against that alias; the real blocker was `sendNotifications`' `concurrency: 8` exceeding the free plan's limit of 5 (fixed, D-97). **Still to verify:** re-sync after the fix deploys and confirm all 10 functions register + a real scheduled run fires. |
