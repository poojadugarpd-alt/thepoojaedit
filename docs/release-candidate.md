...
| 7 | **Vercel project + isolated preview environment — ◐ prod deploy live, custom domain not yet cut over** | AC-17 (isolated preview deploy), AC-18 drills | Vercel project exists (`thepoojaedit`), production deployments are live and building green. **`thepoojaedit.in` currently still serves the owner's existing, real Shopify store** (`CNAME → shops.myshopify.com`) — intentional, confirmed with the owner 2026-09-11: the domain stays on Shopify until this build is ready, then gets repointed as the go-live cutover step. Until that DNS change, the Vercel app is only reachable at its (Vercel-Authentication-protected) `*.vercel.app` URLs — gates blocker 6 above. **Checked 2026-09-13 (read-only):** Function Region is `iad1` (US East) as intended — confirms the "stay on the default US region for now" constraint held, nothing drifted to a paid region. Preview vs Production already get separate env-var scopes by default on the Hobby plan (Vercel's own "Environments" feature — custom named environments beyond the default three — is Pro-only, $50/5, and out of scope per the free-plan constraint; the default Preview/Production isolation already satisfies AC-17's intent without it). **New finding, not yet acted on:** Project Settings → Node.js Version is set to **24.x**, not the 22.x this doc previously assumed — worth a deliberate decision (stay on 24.x, or pin to 22.x) rather than leaving it as an unnoticed default; changing it needs a redeploy and is a real settings change, not done here without the owner's say. |
