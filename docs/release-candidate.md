...
| 3 | **Shadowfax merchant account + supported test arrangement — ◐ code corrected, live test still open** (D-88, 2026-09-11) | **AC-13 (blocked)**, AC-09 live remittance | `shadowfax.ts` rewritten against the account's real live API docs (auth, base URLs, serviceability, marketplace order creation, cancellation, tracking all fixed — see D-88); model confirmed Marketplace/seller-pickup. **Still open:** API token + webhook token into Vercel; one real staging shipment created + tracked + a real push-callback received; label download and COD remittance have no documented self-serve API (`fetchLabel`/`fetchCodRemittance` say so explicitly) — resolve with Shadowfax's account manager before launch |
