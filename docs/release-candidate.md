...
| 4 | **Resend (verified sender + Svix secret) — WhatsApp deferred by owner (2026-09-11), cost** | AC-14 (email leg only) | verify the sending domain; real send to a test recipient. **Meta WhatsApp is out of scope for launch** — approved templates + per-conversation pricing aren't worth it pre-revenue; the notification layer already no-ops a channel with no configured provider (D-75), so this doesn't block anything else. Revisit post-launch. |
...
| 6 | **Inngest project — ◐ created, sync blocked on the domain cutover (2026-09-11)** | the recurring schedules actually firing (they are wired + unit-proven, no live scheduled run yet) | Inngest project + Vercel integration are set up (app `poojaedit`, 10 functions in `src/inngest/functions.ts`, `serve()` route live). **First sync attempt failed** — Inngest reached a Vercel *deployment-specific* URL (`thepoojaedit-<hash>-thepoojaedit.vercel.app`), which 302s to `vercel.com/sso-api` (Vercel Deployment Protection); shows as an "Unattached sync" with 0 functions registered. Owner decision (2026-09-11): don't touch DNS yet — see blocker 7. Re-sync Inngest against `https://thepoojaedit.in/api/inngest` once the domain cutover happens (custom production domains are excluded from Vercel Authentication by default, no bypass secret needed); until then the 10 jobs (shipment/invoice automation, reconciliation, reservation sweep) do not run in production. |
