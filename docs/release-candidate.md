...
| 4 | **Resend — ◐ DNS records live, pending Resend's own verification (2026-09-11); WhatsApp deferred by owner, cost** | AC-14 (email leg only) | Account created; sending domain `mail.thepoojaedit.in` added in Resend (subdomain — independent of the D-94/#7 root-domain cutover); `RESEND_API_KEY` + `EMAIL_FROM` wired into Vercel (all environments). D-95's GoDaddy account mismatch resolved — the owner logged into the correct account and all 3 DNS records (DKIM TXT, SPF MX+TXT) are added and confirmed live via `dig` (D-98); root/Shopify records untouched. Clicked "Verify DNS Records" in Resend (status moved to Pending) but the Resend dashboard went unresponsive before confirming completion — likely a transient Resend-side issue, not expected to need any action; recheck `resend.com/domains`. **Still to do:** confirm verification completed → send a real test email → (at the domain cutover) register the webhook + set `RESEND_WEBHOOK_SECRET`. See D-98. **Meta WhatsApp is out of scope for launch** — approved templates + per-conversation pricing aren't worth it pre-revenue; the notification layer already no-ops a channel with no configured provider (D-75), so this doesn't block anything else. Revisit post-launch. |
