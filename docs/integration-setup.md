...
| Resend | 10 | Test sending + verified domain | ◐ | **Code slice done (Phase 10)** — transport (REST), `/api/webhooks/resend` (Svix), all tested against `FakeEmailTransport`. **Account + DNS done 2026-09-11:** Resend account created; sending domain added as `mail.thepoojaedit.in` (subdomain, so it never touches the root/`www` DNS the live Shopify store uses); `RESEND_API_KEY` (Sending-access scope) + `EMAIL_FROM` (`The Pooja Edit <notifications@mail.thepoojaedit.in>`) wired into Vercel (all 3 environments). The D-95 GoDaddy account mismatch is resolved — all 3 DNS records (DKIM TXT, SPF MX+TXT) are added and confirmed live via `dig` (D-98); "Verify DNS Records" clicked in Resend (status → Pending) but the dashboard went unresponsive before confirming completion, likely transient — recheck `resend.com/domains`. Until verification completes, `resendEmailTransport().configured` is true but every send 4xxs (caught as a channel failure per D-91/D-75, not a crash). **Still to do:** confirm verification completed in Resend → send a real test email → then add the webhook endpoint and set `RESEND_WEBHOOK_SECRET` (this last step still needs the domain cutover, same as Inngest — D-94). |
