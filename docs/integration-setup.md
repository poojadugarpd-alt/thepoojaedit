...
| Resend | 10 | Test sending + verified domain | ◐ | **Code slice done (Phase 10)** — transport (REST), `/api/webhooks/resend` (Svix), all tested against `FakeEmailTransport`. **Account done 2026-09-11:** Resend account created; sending domain added as `mail.thepoojaedit.in` (subdomain, so it never touches the root/`www` DNS the live Shopify store uses) — DNS records generated (DKIM TXT, SPF MX+TXT) but **not yet added**, because `thepoojaedit.in` turned out to be registered under a different GoDaddy account than the one signed into in this session (D-95); `RESEND_API_KEY` (Sending-access scope) + `EMAIL_FROM` (`The Pooja Edit <notifications@mail.thepoojaedit.in>`) are wired into Vercel (all 3 environments) so sending goes live the moment the domain verifies — until then `resendEmailTransport().configured` is true but every send 4xxs (caught as a channel failure per D-91/D-75, not a crash). **Still to do:** find/log into the correct GoDaddy account → add the 3 DNS records → verify in Resend → send a real test email → then add the webhook endpoint and set `RESEND_WEBHOOK_SECRET` (this last step also needs the domain cutover, same as Inngest — D-94). |
